...
namespace InGameTextEditor
{
    ...
    public class TextEditor : MonoBehaviour
    {
        ...
        public void prepForExecution(int count)
        {
            //EditorActive = false;
            //EditorActive = true;
            Debug.Log("prep");
            ...
            InsertText(new TextPosition(count,0), "\n#", false,true);
            //editorActive = false;
            //RebuildLines();
            InsertCharacter('\n');
            lines.Add(new Line(lines.Count, " ", 0f, this));
            //int output = RebuildLinesOutput(true);
            //Debug.Log("return: " + output);
            UpdateLayout();
            editorActive = true;
        }
        ...
        void Update()
        {
            ...
            do
            {
                ...
                //Debug.Log("op update: " + op.ToString());
            ...
            if (editorActive && editorActiveFrames > 1)
            {
                ...
                {
                    // macOS key binding
                    if ((Input.GetKey(KeyCode.LeftCommand) || Input.GetKey(KeyCode.RightCommand)) && !Input.GetKey(KeyCode.AltGr))
                        ctrlOrCmdPressed = true;
                }
                else
                {
                    // Windows key binding
                    if ((Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl)) && !Input.GetKey(KeyCode.AltGr))
                        ctrlOrCmdPressed = true;
                }
        ...
        // rebuilds all text lines
        int RebuildLinesOutput(bool immediately = false)
        {
            // creation operation
            RebuildLinesOperation op = new RebuildLinesOperation();
